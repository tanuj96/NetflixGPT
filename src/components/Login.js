  ...
  return (
    <div>
      ...
      {!isSignIn && <input type="text" placeholder="Full Name" className="p-4 my-4 w-full bg-gray-700"/> }
        ...
        <button className="font-bold p-4 my-6 bg-red-700 w-full rounded-lg">{isSignIn? 'Sign In' : 'Sign Up'}</button>
        <p className="py-6 cursor-pointer no-underline hover:underline" onClick={toggleSignInForm}>{isSignIn? 'New to Netflix? Sign Up Now' : 'Existing User? Sign In Now'}</p>
      </form>
    </div>
  );
};
